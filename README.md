...
作为前端人员，学习 nodejs 是必须要懂的。然而，教程都是前后台集中在一起，本项目是自己抽离出来的项目，专门给前端提供接口和数据库操作。并没有大多 express 教程，搞一堆模版
...
新建 user.js, 这里的逻辑就是接口和数据口的逻辑，三个接口 /login,  /register, /userInfo, /logout 四个接口
...
npm start 
 ...
 1、启动数据库
 2、启动项目
 3、打开数据可视化 robo3t 创建连接数据库那段代码定义的 数据集名称， 如果已有就不用建了
 4、发送请求，验证是否成功
